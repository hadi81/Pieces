...
def adjust_sections(linker_script, section_info, project_binary_path):
	...
	while index < len(lines):
		...
		if ("rtmkcode" in line and ":" in line):
			...
			rtmk_size = section_info[first_item_line]["size"]
			crtmk_lma = location_counter
			crtmk_vma = location_counter

			fixed_rtmk_size, fixed_crtmk_vma, fixed_crtmk_lma = fixLMAAndVMA(rtmk_size, crtmk_vma, crtmk_lma)	
			offset = fixed_crtmk_lma-crtmk_lma

			compartment_code_offset = compartment_code_offset + offset
			line = first_item_line + f" (CODE_SECTIONS_START + COMPARTMENT_CODE_COUNTER + {hex(compartment_code_offset)} ) : AT(CODE_SECTIONS_START + COMPARTMENT_CODE_COUNTER + {hex(compartment_code_offset)} )\n"
			new_file_lines[-1] = line

			end_location_counter = f"\t\t . =  (CODE_SECTIONS_START + COMPARTMENT_CODE_COUNTER + {hex(compartment_code_offset)} ) + {hex(fixed_rtmk_size)};\n"
			...
			new_file_lines.insert(len(new_file_lines) - 2, end_location_counter)
			compartment_code_counter = compartment_code_counter + offset + fixed_rtmk_size

			counter_line = "COMPARTMENT_CODE_COUNTER = COMPARTMENT_CODE_COUNTER + SIZEOF(.rtmkcode);"
			new_file_lines.append(counter_line)

		# if ("rtmkdata" in line and ":" in line):
		# 	location_counter_base=code_sections_start+compartment_code_counter
		# 	location_counter = location_counter_base

		# 	rtmkdata_size = section_info[".rtmkdata"]["size"]
		# 	drtmk_lma = location_counter
		# 	drtmk_vma = oprev_vma_end_addr

		# 	fixed_rtmkdata_size, fixed_drtmk_vma, fixed_drtmk_lma = fixLMAAndVMA(rtmkdata_size, drtmk_vma, drtmk_lma)	
		# 	offset = fixed_drtmk_lma-drtmk_lma
		# 	print("=========================")
		# 	print(fixed_rtmkdata_size)
		# 	compartment_code_offset = compartment_code_offset + offset

		# 	line = f".rtmkdata {fixed_vma} : AT(CODE_SECTIONS_START + COMPARTMENT_CODE_COUNTER + {hex(compartment_code_offset)} )\n"
		# 	new_file_lines[-1] = line
		...
		# 	end_location_counter = f"\t\t . =  {fixed_vma + fixed_rtmkdata_size} ;\n"
		# 	new_file_lines.insert(len(new_file_lines) - 3, end_location_counter)
